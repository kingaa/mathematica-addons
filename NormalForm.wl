...
BeginPackage["NormalForm`", {"Frechet`", "Taylor`", "VFTransform`"}]

Unprotect[
   NormalForm, ResonanceTest, Form, ForwardAdjointAction,
   BackwardAdjointAction, ForwardAction, BackwardAction,
   Jordan, Complexification, Realification,
   Generator, Semisimple, Resonance
]

NormalForm::usage = "{Y,U} = NormalForm[X,vars,order] reduces the vectorfield jet X to its normal form using a Lie series method.  It is assumed that the linear part of the jet X is in Jordan normal form.  Y is the normalized vectorfield jet and U the vectorfield jet that generates the inverse of the normalizing transformation.  Thus, if u is the transformation generated by U, (i.e., u = ForwardAction[vars,U,vars,order]), we have Du . X = Y o u, and, if v = BackwardAction[vars,U,vars,order], then Dv . Y = X o v.";
...
NormalForm[X_List, vars_List, order_Integer, opts___] :=
   Module[
...
NormalFormAux[X_List, vars_List, order_Integer, lieSolve_, zeroTest_] :=
   Block[
      ...
      Module[
         ...
         F[i_Integer/;(i > 0), m_Integer] := F[i,m] =
         Expand[
            F[i-1,m+1] -
            Sum[
         ...
         {
            Expand[
               Sum[ F[i,0]/i!, {i,0,order-1}]
            ],
            Expand[
               Sum[ Y[i]/i!, {i,0,order-2}]
            ]
         }
      ]
   ]
...
ForwardAdjointAction[X_List, Y_List, vars_List, order_Integer] :=
   Module[
      ...
      F[i_Integer, m_Integer/;(m > 0)] := F[i,m] =
      Expand[
         F[i+1,m-1] +
         Sum[
...
ForwardAction[X_, Y_List, vars_List, order_Integer] :=
   Module[
      ...
      Expand[
         F[i+1,m-1] +
         Sum[
            Binomial[i,j] LieDeriv[U[j],F[i-j,m-1],vars],
            {j,0,i}
         ]
      ];
      Expand[
         Coefficient[Xe, eps, 0] +
         Sum[ F[0,m] / m!, {m,0,order-1}]
      ]
   ]
...
BackwardAction[X_, Y_List, vars_List, order_Integer] :=
   Module[
      ...
      Expand[
         F[i-1,m+1] -
         Sum[
            Binomial[i-1,j] LieDeriv[U[j],F[i-j-1,m],vars],
            {j,0,i-1}
         ]
      ];
      Expand[
...
Generator[f_List, vars_List, n_Integer] :=
   Module[
      {Y, eps, fe},
      ...
      Y[i-1,k+1] -
      Sum[
         Binomial[i-1, j] LieDeriv[Y[i-j-1,0],Y[j,k],vars],
         {j,0,i-1}
      ];
...
Jordan[X_List, oldvars_List, newvars_List] :=
   Module[
...
FieldSSSolve[_, eigs_List, X_List, vars_List, _, zero_] :=
   Transpose[
      Table[
...
FieldSSSolveAux[eigs_List, a_, vars_, lambda_, zero_] :=
   Module[
      {divisor = lambda - eigs . Exponent[a,vars]},
      If[ zero[divisor] == 0,
          {a,0},
          {0,a/divisor},
          {0,a/divisor}
...
monoms[order_Integer/;(order > 0), {x_, y___}] :=
   Flatten[
      Table[
