...
Unprotect[
   Ideal, IdealSum, IdealProduct, IdealQuotient,
   IdealIntersection, Saturation, EliminationIdeal, IdealReduce,
   MultiIndices, Monomials, HomogenousForm,
...
HomogeneousForm::usage = "HomogeneousForm[k, vars, A, n] returns a generic homogeneous form of degree k in the variables vars.  The coefficients are given in terms of the symbol A.  If n > 1, an n-vector is returned.";
...
EliminationIdeal::usage = "EliminationIdeal[F, elim, vars] computes the elimination ideal with respect to the variables in elim. If <G> is this ideal, then <G> is the intersection of <F> and k[vars\elim].";
...
MultiIndices[order_Integer/;(order > 0), n_Integer/;(n > 1)] :=
   Join @@
   Table[
...
HomogeneousForm[degree_Integer, vars_List, coef_Symbol, n_Integer: 1] :=
   Module[
      {indices, monoms, coefs},
      indices = MultiIndices[degree, Length[vars]];
      monoms = (Times @@ (vars^#)) & /@ indices;
      coefs = If[ n == 1,
                  coef /@ indices,
                  Table[coef[i, #] & /@ indices, {i, 1, n}]
              ];
      coefs . monoms
   ]
...
IdealIntersection[F_List, G_List, X_List, opts___] :=
   Module[
      {t, H},
      H = GroebnerBasis[
         Join[t F, (1 - t) G],
...
IdealQuotient[F_List, G_List, X_List, opts___] :=
   Module[
      {K, L},
...
EliminationIdeal[F_List, Y_List, X_List, opts___] :=
   Module[
      {H, cond},
      H = Ideal[F,X,opts];
      cond = Outer[!MemberQ[Variables[#1],#2]&,H,Y];
      Pick[H, (And @@ #)& /@ cond]
   ]
...
Saturation[F_List, G_, X_List, opts___] :=
   Module[
      {t, H},
      H = GroebnerBasis[
         Append[F, 1 - t G],
...
StandardBasis[F_List, X_List, opts___] :=
   Module[
      {z, G},
      G = GroebnerBasis[
         Homogeneize[F,X,z],
...
SeriesReduce[f_, F_List, X_List, opts___] :=
   Module[
      {z, H},
      H = PolynomialReduce[
         Homogeneize[f,X,z],
...
TangentCone[F_List, X_List, opts___] :=
   GroebnerBasis[
